param(
    ...
    [string]$OutputPath = (Join-Path (Join-Path $PSScriptRoot "..") (Join-Path "content" "models" "establishment-type-field-rules.md"))
)
...
$lines = @(
    ...
    "",
    "This page lists EPR target validation rules by establishment type. It is generated from ``models/establishment-details-shacl.ttl`` during the site build. Do not edit this file directly.",
    "",
    "**This page is not a complete inventory of current GIAS Details-tab fields.** It covers structural constraints — which fields must, must not, or may be present for each establishment type. Address and contact fields (headteacher, website, telephone) appear here as universal constraints. Value-level rules for dates, pupil numbers and capacity are in the separate [data quality rules page](../establishment-data-quality-rules/). Fields not yet constrained in either file include Ofsted and external service links, PRU-specific operational fields, online provider accreditation fields, and several other areas flagged as requiring design decisions.",
    "",
    "Each section corresponds to a SHACL shape in the source file. Group shapes cover all types in a family. Type-specific shapes add to or override the group rules.",
    "",
    "| Rule | Meaning |",
    "| --- | --- |",
    "| Required — exactly one | The field must be present with exactly one value in an EPR record |",
    "| Required — at least one | The field must be present with at least one value in an EPR record |",
    "| Not applicable | The field must not be present for this establishment type in EPR. Some fields marked not applicable are visible in the current GIAS Details tab — the rule reflects the EPR target, not current GIAS behaviour. |",
    "",
    "**Note on 'Local authority — accountability relationship (not applicable)':** This rule means the establishment type has no accountability relationship with a local authority in the EPR model. It does not mean the local authority geographic or administrative row is absent from the GIAS Details tab — that row appears for all establishment types in the current GIAS UI.",
    ""
)
